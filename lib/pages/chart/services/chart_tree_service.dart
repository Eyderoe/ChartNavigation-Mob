...
class ChartTreeService {
  const ChartTreeService();

  static const String rootFolderName = '航图';

  Future<List<ChartFileNode>> loadTree() async {
    ...
    final List<ChartFileNode> nodes =
        entities.map(_buildNode).whereType<ChartFileNode>().toList()
          ..sort(_compareNodes);
    _log(
  ...
  ChartFileNode? _buildNode(FileSystemEntity entity) {
    ...
    if (entity is File) {
      ...
      return ChartFileNode(name: name, path: path, isDirectory: false);
    }

    if (isDirectory) {
      final List<ChartFileNode> children =
          entity.listSync().map(_buildNode).whereType<ChartFileNode>().toList()
            ..sort(_compareNodes);
  ...
  int _compareNodes(ChartFileNode a, ChartFileNode b) {
  ...
  void _log(String message) {
    debugPrint('[ChartTreeService] $message');
  }
}
